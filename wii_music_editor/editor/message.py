...
class TextClass:
    ...
    def read(self):
        ...
        with open(
            self.__filepath / self.__folder / "new_music_message.txt", "rb"
        ) as message:
        ...
        for style in style_list:
            if (
                style.style_type == StyleType.Global
                or style.style_type == StyleType.QuickJam
            ):
    ...
    def change_name(
        self, item: SongClass or Style, new_text: list[str], encode: bool = True
    ):
        ...
        for i in range(len(offsets)):
            ...
            for j, text in enumerate(self.textlines):
                if offset in text.decode("utf-8"):
                    ...
                    text_to_add = (
                        repr(new_text[i]).strip("'").replace(r"\'", "'").strip('"')
                    )
                    self.textlines[j] = bytes(
                        f"{offset}{text.decode('utf-8')[10:22]}{text_to_add}\r\n",
                        "utf-8",
                    )
                    break
    ...
    def encode(self):
        try:
            ...
            with open(
                self.__filepath / self.__folder / "new_music_message.txt", "wb"
            ) as message:
    ...
    def fix_message_file(self):
        for num in range(len(self.textlines)):
            if self.textlines[num] == b"  b200 @015f /\r\n":
                self.textlines[num] = (
                    b"  b200 @015f [/,4b] = "
                    + StyleNames.default[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 1] = (
                    b"  b201 @0160 [/,4b] = "
                    + StyleNames.rock[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 2] = (
                    b"  b202 @0161 [/,4b] = "
                    + StyleNames.march[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 3] = (
                    b"  b203 @0162 [/,4b] = "
                    + StyleNames.jazz[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 4] = (
                    b"  b204 @0163 [/,4b] = "
                    + StyleNames.latin[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 5] = (
                    b"  b205 @0164 [/,4b] = "
                    + StyleNames.reggae[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 6] = (
                    b"  b206 @0165 [/,4b] = "
                    + StyleNames.hawaiian[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 7] = (
                    b"  b207 @0166 [/,4b] = "
                    + StyleNames.electronic[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 8] = (
                    b"  b208 @0167 [/,4b] = "
                    + StyleNames.classical[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 9] = (
                    b"  b209 @0168 [/,4b] = "
                    + StyleNames.tango[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 10] = (
                    b"  b20a @0169 [/,4b] = "
                    + StyleNames.pop[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                self.textlines[num + 11] = (
                    b"  b20b @016a [/,4b] = "
                    + StyleNames.japanese[self.__language].encode("utf-8")
                    + b"\r\n"
                )
                break
