...
///
/// # Errors
/// Returns `WmError::Settings` if the file cannot be written.
pub fn save_setting(path: &Path, section: &str, key: &str, value: &str) -> Result<(), WmError> {
    let mut config = Ini::new();
    // Load existing or start fresh — preserves other sections/keys
    ...
    config
        ...
        .map_err(|e| WmError::Settings(e.to_string()))
}
