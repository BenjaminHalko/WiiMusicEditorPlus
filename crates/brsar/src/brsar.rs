...
impl Brsar {
    /// Parses a BRSAR file image.
    ///
    /// # Errors
    /// Returns `BrsarError::Parse` if the data is too short, has invalid magic,
    /// or contains section offsets outside the file.
    pub fn parse(data: Vec<u8>) -> Result<Self, BrsarError> {
    ...
    /// Reads a song blob by flat item index.
    ///
    /// # Errors
    /// Returns `BrsarError::Parse` if the song index is invalid or the song range
    /// would extend past the file.
    pub fn get_song(&self, item: usize) -> Result<Vec<u8>, BrsarError> {
    ...
    /// Reads a song blob from a specific group and item.
    ///
    /// # Errors
    /// Returns `BrsarError::Parse` if the group/item index is invalid or the song
    /// range would extend past the file.
    pub fn get_song_in_group(
        &self,
        group_index: usize,
        item_index: usize,
    ) -> Result<Vec<u8>, BrsarError> {
        let groups = self.parse_groups()?;
        let (group, item_entry) = Self::resolve_group_item(&groups, group_index, item_index)?;
        let rseq_offset = group.rseq_offset + item_entry.rseq_offset;
        let rseq_end = rseq_offset
            .checked_add(item_entry.rseq_size)
            .ok_or_else(|| Self::parse_error(item_entry.entry_offset, "song range overflow"))?;
    ...
    /// Replaces a song blob by flat item index.
    ///
    /// # Errors
    /// Returns `BrsarError::Parse` if the song index is invalid, the song range
    /// would extend past the file, or size math overflows.
    pub fn replace_song(&mut self, item: usize, new_data: &[u8]) -> Result<(), BrsarError> {
        let groups = self.parse_groups()?;
        let (group_index, item_index) = Self::resolve_item(&groups, item)?;
    ...
    /// Replaces a song blob from a specific group and item.
    ///
    /// # Errors
    /// Returns `BrsarError::Parse` if the group/item index is invalid, the song
    /// range would extend past the file, or size math overflows.
    pub fn replace_song_in_group(
        &mut self,
        group_index: usize,
        item_index: usize,
        new_data: &[u8],
    ...
    fn parse_groups(&self) -> Result<Vec<GroupDataEntry>, BrsarError> {
    ...
    fn resolve_item(groups: &[GroupDataEntry], item: usize) -> Result<(usize, usize), BrsarError> {
    ...
    fn resolve_group_item(
        groups: &[GroupDataEntry],
        group_index: usize,
        item_index: usize,
    ) -> Result<(&GroupDataEntry, &GroupItemEntry), BrsarError> {
        let group = groups
            .get(group_index)
            .ok_or_else(|| Self::parse_error(0x00, "group index out of range"))?;
        let item_entry = group
            .items
            .get(item_index)
            .ok_or_else(|| Self::parse_error(0x00, "group item index out of range"))?;
        Ok((group, item_entry))
    }

    fn read_u32_checked(&self, offset: usize, context: &str) -> Result<u32, BrsarError> {
        let end = offset
    ...
    fn parse_error(offset: usize, message: &str) -> BrsarError {
        BrsarError::Parse {
            offset,
